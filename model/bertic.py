...
def oversample_classes(train_data, label_encoder, target_class):
    ...
    sampling_strategy = {
        label_index: class_counts[target_class_index]
        for label_index in class_counts.index if label_index != target_class_index
    }
...
combined_data = pd.read_csv('eu_combined_final.csv')
...
combined_data['Speaker_party_name'] = combined_data['Speaker_party_name'].replace('-', 'Brez')
...
for fold, (train_idx, test_idx) in enumerate(skf.split(combined_data, combined_data['labels'])):
    ...
    filename = f'fold_{fold + 1}_speeches.txt'
...
plt.savefig('/shared/home/andela.rajovic/eu/bertic/confusion_matrix_eu_slovenian.png')
...
results_df.to_csv('/shared/home/andela.rajovic/eu/bertic/model_predictions_analysis.csv', index=False)
...
plot_stance_distribution_party(combined_data, '/shared/home/andela.rajovic/eu/bertic/stance_distribution_by_party_grouped.png')
...
def plot_gender_distribution_separate_bars(data, output_path):
    ...
    plt.savefig(output_path)
    plt.show()
plot_gender_distribution_separate_bars(combined_data, '/shared/home/andela.rajovic/eu/bertic/stance_distribution_by_gender_separate_bars.png')
...
plot_age_group_distribution_separate_bars(combined_data, '/shared/home/andela.rajovic/eu/bertic/stance_distribution_by_age_group_separate_bars.png')
